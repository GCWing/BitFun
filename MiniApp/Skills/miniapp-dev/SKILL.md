---
name: miniapp-dev
description: Develops, maintains, and generates BitFun Live Apps (Zero-Dialect Runtime). Use when (1) working on live_app framework code under src/crates/core/src/live_app/ or src/web-ui/src/app/scenes/apps/live-app/; or (2) generating / creating / designing a NEW Live App for the user — including any request like "做一个小应用 / 生成 Live App / 写个 BitFun 小工具 / 创建 mini app". Also triggers on Live App, miniapps, bridge, zero-dialect, InitLiveApp, app.fs / app.shell / app.storage, or any work under MiniApp/Demo/ and MiniApp/Skills/.
---
...
2. **找设计上下文**：先读 `MiniApp/Demo/` 与 `src/crates/core/src/live_app/builtin/assets/` 中**最贴近形态**的内置应用，复刻它的视觉语言（间距 / 圆角 / 卡片密度 / motif）。**从零 mock 是最后选择**。
