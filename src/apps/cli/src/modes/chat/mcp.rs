...
fn mcp_add_completion_message(name: &str) -> String {
        ...
        while index < self.pending_mcp_tasks.len() {
            ...
            let (success_message, failure_context, result) = match task {
                ...
                PendingMcpTask::Add { name, handle } => (
                    Some(mcp_add_completion_message(&name)),
...
mod mcp_terminal_tests {
    ...
    #[test]
    fn add_reports_the_host_owned_server_started() {
        assert_eq!(
            mcp_add_completion_message("srv"),
