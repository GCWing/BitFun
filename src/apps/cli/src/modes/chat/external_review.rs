...
fn external_agent_review_text(snapshot: Option<&ExternalSourceCatalogSnapshot>) -> String {
    let Some(snapshot) = snapshot else {
        return "Agents\n\nExternal AI applications\nBitFun has not finished checking imported agents. Run /agents refresh and try again."
            .to_string();
    };
    let mut lines = vec![
        "Agents".to_string(),
        ...
        for (index, agent) in snapshot.subagents.iter().enumerate() {
            ...
            match agent.activation_state {
                ...
                | ExternalSubagentActivationState::Declined => {
                    lines.push(format!("     Command: /agents enable {}", index + 1))
                }
                ExternalSubagentActivationState::Active => {
                    lines.push(format!("     Command: /agents disable {}", index + 1))
                }
                _ => {}
            }
        }
    ...
    if pending_conflicts.is_empty() {
        ...
        for (conflict_index, conflict) in pending_conflicts.iter().enumerate() {
            ...
            lines.push(format!(
                "     Keep unavailable: /agents choose {} 0",
                conflict_index + 1
            ));
            lines.push(
    ...
    lines.push(String::new());
    lines.push(
        "Run /agents refresh after editing, upgrading, or removing agent configuration in an external AI application."
            .to_string(),
    );
    lines.join("\n")
}
...
fn parse_external_agent_review_action(
    ...
    if command.eq_ignore_ascii_case("refresh") {
        if parts.next().is_some() {
            return Err("usage: /agents refresh".to_string());
        }
    ...
    let snapshot = reviewed_snapshot.or(current_snapshot).ok_or_else(|| {
        "BitFun has not finished checking agents from external AI applications; run /agents refresh".to_string()
    })?;
    if command.eq_ignore_ascii_case("enable") || command.eq_ignore_ascii_case("disable") {
        ...
        if parts.next().is_some() {
            return Err(format!("usage: /agents {command} <agent-number>"));
        }
        let agent = snapshot
            .subagents
            .get(index)
            .ok_or_else(|| "that agent is no longer available; run /agents refresh".to_string())?;
        ...
        if !allowed {
            return Err(format!(
                "agent {} is {}; run /agents refresh for its next step",
                index + 1,
                external_agent_activation_label(&agent.activation_state)
            ));
        }
    ...
    if command.eq_ignore_ascii_case("choose") {
        ...
        if parts.next().is_some() {
            return Err("usage: /agents choose <conflict-number> <choice-number>".to_string());
        }
        let conflict = snapshot
            ...
            .nth(conflict_index)
            .ok_or_else(|| {
                "that conflict is no longer available; run /agents refresh".to_string()
            })?;
        let (candidate_id, approve_external) = if candidate_number == 0 {
            ...
            let candidate = conflict
                ...
                .ok_or_else(|| {
                    "that choice is no longer available; run /agents refresh".to_string()
                })?;
    ...
    Err("usage: /agents [refresh | enable <number> | disable <number> | choose <conflict-number> <choice-number>]".to_string())
}
    ...
    match action {
        ...
        ExternalAgentReviewAction::Decide {
            ...
            match (approved, active.map(|agent| &agent.activation_state)) {
                ...
                _ => {
                    "External agent decision saved; run /agents refresh to review its current state"
                        .to_string()
                }
            }
        }
        ExternalAgentReviewAction::Choose {
            ...
            if selected {
                ...
                "Agent choices changed; run /agents refresh before choosing".to_string()
            }
        }
