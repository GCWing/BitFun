 ...
 *
 * Shows a global enable/disable toggle and a list of discovered plugins
 * with per-plugin enable/disable switches.
 */
...
export const PluginsConfig: React.FC = () => {
  ...
  const loadStatus = useCallback(async () => {
    try {
      setLoading(true);
      const result = await pluginAPI.getPluginStatus();
      setStatus(result);
    } catch (err) {
  ...
  const handleGlobalToggle = async (enabled: boolean) => {
    try {
      const result = await pluginAPI.setPluginsEnabled(enabled);
      setStatus(result);
    } catch (err) {
  ...
  const handlePluginToggle = async (plugin: PluginStatusView, trusted: boolean) => {
    ...
    try {
      ...
      // Refresh full status after changing trust.
      const result = await pluginAPI.getPluginStatus();
      setStatus(result);
    } catch (err) {
  ...
  return (
    <ConfigPageLayout>
      ...
      <ConfigPageContent>
        ...
        {/* Plugin list */}
        {pluginsEnabled && (
          <ConfigPageSection title={t('pluginListSection')}>
            {plugins.length === 0 ? (
