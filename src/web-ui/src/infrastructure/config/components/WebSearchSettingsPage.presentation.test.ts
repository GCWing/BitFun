...
describe('WebSearch settings presentation', () => {
  it('keeps secrets out of generic config writes and places provider selection in the section header', () => {
    ...
    expect(source).toContain('className="web-search-settings__provider-select"');
    expect(source).toContain("variant={hasUnsavedChanges ? 'primary' : 'quiet'}");
    expect(source).toContain("aria-label={t('actions.saveConfiguration')}");
    ...
    expect(source).toContain('className="web-search-settings__credential-field"');
    expect(source.match(/ balanced>/g)).toHaveLength(4);
