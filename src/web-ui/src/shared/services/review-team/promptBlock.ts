...
export function buildReviewTeamPromptBlockContent(
  ...
  return [
    ...
    '- When a configured member entry provides model_id, pass model_id with that value to the matching LaunchReviewAgent call.',
    ...
    '- If a reviewer fails or times out without useful partial output, retry that same reviewer at most max_retries_per_role times: reduce its scope, downgrade strategy by one level when possible, use a shorter timeout, and set retry to true on the retry LaunchReviewAgent call.',
