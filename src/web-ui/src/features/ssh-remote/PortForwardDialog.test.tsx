...
describe('PortForwardDialog', () => {
  ...
  it('detects remote ports without forwarding any of them', async () => {
    sshApiMock.listRemoteListeningPorts.mockResolvedValue([
      { port: 5173, bindAddress: '127.0.0.1', process: 'vite' },
    ]);
    await renderDialog();

    await act(async () => {
      click('ssh-port-forward-detect');
      await Promise.resolve();
    });

    expect(container.textContent).toContain('5173');
    expect(sshApiMock.startPortForward).not.toHaveBeenCalled();
  });
