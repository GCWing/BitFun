...
- When using Bash, choose `timeout_ms` for commands that may block, prompt for input, install dependencies, use the network, or run tests/builds. Use `run_in_background` for servers, watchers, REPLs, and commands intended to keep running. Omit `timeout_ms` only when you intentionally want to wait without a tool-imposed timeout.
