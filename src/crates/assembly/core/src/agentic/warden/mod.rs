...
//!
//! # Design note (2026-08-09): Warden is a permanent part of the agentic
//! runtime core, not an optional feature
//!
//! The Warden runtime is embedded in the scheduler (`warden_runtime` field,
//! constructed unconditionally) and the tool pipeline (190+ references), and
//! `AgentRole::Warden` is part of the RBAC role enum. It therefore compiles
//! unconditionally whenever `agent-runtime` is enabled, and its `rand`
//! dependency is owned by the `agent-runtime` feature (same pattern as
//! md5/similar/rusqlite). Do not try to gate this module behind a separate
//! feature (`warden-poke` is an empty alias only) — that would require
//! conditionally compiling the scheduler/tool-pipeline core.
...
impl ShameWallRegistry {
    ...
    ///
    /// The write is **atomic**: the JSON is first written to a unique
    /// temporary file in the same directory (`.shame-wall-registry.json.<rand>`
    /// suffix) and then renamed over the destination. A crash or power loss
    /// mid-write can therefore never leave a truncated registry at the target
    /// path — readers either see the previous complete registry or the new
    /// one, never a partial file (d1-P2-3). On failure the temp file is
    /// removed so no garbage accumulates.
    pub fn save_to_path(&self, path: &std::path::Path) -> BitFunResult<()> {
        ...
        if let Err(err) = std::fs::rename(&tmp_path, path) {
            let _ = std::fs::remove_file(&tmp_path);
            return Err(BitFunError::io(format!(
                "failed to atomically replace shame-wall registry at {}: {}",
                path.display(),
                err
            )));
        }
...
/// Build a unique sibling temp path for an atomic file replacement.
...
///
/// Audit trace (d1-P1-3, defense-in-depth): penalties requested under this
/// marker are recorded on the shame wall with `agent_pattern =
/// "warden-runtime-internal"` (see `PunishmentExecutor::agent_pattern_for`),
/// so scheduler-embedded penalties are distinguishable from user-facing
/// Warden-session penalties and never blend into the audit stream.
...
/// - Maximum consecutive defer count is 3. After the 3rd consecutive defer,
///   the *next* defer request is rejected: the agent must complete at least
///   one work turn (which resets the count) before it may defer again.
///   (d1-P2-1: this phrasing is deliberately precise — with the default
///   [`MAX_DEFER_COUNT`] of 3, `track_defer` allows defers 1-3 and rejects
///   the 4th, i.e. "after 3 defers the next defer is refused".)
...
impl PokePriorityManager {
    ...
    ///
    /// Increments the defer counter for `session_id`. Returns `true` when the
    /// defer is no longer allowed: the count now exceeds `max_defer_count`,
    /// i.e. this is the (max_defer_count + 1)-th consecutive defer. With the
    /// default [`MAX_DEFER_COUNT`] of 3, the 4th consecutive defer is the
    /// first one to be rejected — the 1st..3rd consecutive defers are all
    /// allowed, and completing a work turn resets the count (see
    /// [`Self::reset_defer_count`]).
    ///
