...
impl PunishmentExecutor {
    ...
    /// Shame-wall `agent_pattern` for a penalty source.
    ///
    /// The internal [`super::WARDEN_RUNTIME_SESSION`] source is labelled
    /// `warden-runtime-internal` so audit records can distinguish scheduler-
    /// embedded penalties (which bypass the daemon check) from user-facing
    /// Warden-session penalties (labelled `agent`).
    fn agent_pattern_for(requested_by: &str) -> &'static str {
        if requested_by == super::WARDEN_RUNTIME_SESSION {
            "warden-runtime-internal"
        } else {
            "agent"
        }
    }
    ...
    ///
    /// Defense-in-depth: the short-circuit is not silent — the internal source
    /// is recorded in the shame-wall entry via `agent_pattern =
    /// "warden-runtime-internal"` (see [`Self::agent_pattern_for`]) so every
    /// penalty that bypasses the daemon check leaves an auditable trace
    /// instead of blending into the user-facing Warden session stream.
    ...
    async fn execute_l1(
        ...
        shame_wall.upsert_entry(
            ...
            Self::agent_pattern_for(&request.requested_by),
