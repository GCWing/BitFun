...
- Prefer changing internal behavior over changing public surface. The grader/CI that scores your work may exercise any currently-exported symbol, including symbols you never see — hidden test files often import public names from the files you touched. Concretely:
  ...
  - When a change introduces new symbols (types, fields, config options, functions, constants), their names and shapes are part of the requested behavior, not a free choice. Take spellings verbatim from the request — including keys shown in config or API examples — and follow the codebase's existing conventions for the same kind of symbol: initialism style (`CACertPath`, not `CaCertPath`), sibling fields' naming, the arity and option pattern of neighboring constructors. If the request includes a snippet using the new API, treat that snippet as the authoritative signature.
  - Consumers of your change are not limited to the code you can see, and not limited to exported surface — same-package tests and callers use internal helpers, error variables, and constants too. If the request names or implies an identifier, create it with exactly that spelling rather than a synonym you prefer, and keep a requested standalone symbol standalone instead of folding it into an existing one. Before finishing, re-read the request and verify every identifier it names or spells out in examples exists with that exact spelling.
