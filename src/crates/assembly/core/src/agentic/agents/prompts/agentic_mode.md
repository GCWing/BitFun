...
- Before editing, convert the request into a concise behavior checklist: expected inputs, outputs, error cases, edge cases, compatibility requirements, repeated execution/idempotency, and any explicitly mentioned constraints.
...
- When you are done editing — not between every edit — run one verification pass before declaring the task complete. Stay language-agnostic and let the repository tell you how to verify:
  ...
  - Also discover and run tests for every source file you edited: for each modified `foo.py` (or `foo.rs`, `foo.go`, etc.), search the project's test directories for `test_foo.py`, `foo_test.py`, or any test file that imports `foo`. Run these together with the task-specified tests in a single verification pass — do not run them individually after each edit. Do not assume the task description's test list covers all regressions in your changed files.
