...
impl AcpAgent {
    pub fn new(client_id: String, display_name: String) -> Self {
        ...
        // ACP agents get the unified subagent tool set
        // (subagent_default_tools = shared_coding_mode_tools + SessionControl),
        // so delegated ACP sessions are not limited to a read-only 4-tool
        // baseline.
...
mod tests {
    ...
    fn acp_agent_default_tools_match_agentic_plus_forwarding_tool() {
        ...
        // Same unified subagent tool set (agentic full tools + SessionControl)...
        let mut expected = subagent_default_tools();
