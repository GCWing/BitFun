...
///
/// Scope matching is case-insensitive: "OPENED", "Recent", and
...
///
/// `status` is emitted in lowercase (`active`, `inactive`, ...) to mirror the
/// `WorkspaceScan` input contract (`by_status:active` etc. — d6-P2-3), so a
/// returned status can be fed straight back into a follow-up scoped scan.
fn workspace_info_to_entry(info: &WorkspaceInfo) -> Value {
    json!({
        ...
        "workspaceType": info.workspace_type.as_str(),
    })
}
...
fn workspace_summary_to_entry(summary: &WorkspaceSummary) -> Value {
    json!({
        ...
        "workspaceType": summary.workspace_type.as_str(),
    })
}
...
Each returned entry has the shape {id, name, rootPath, status, openedAt, lastAccessed, workspaceType}. `status` is emitted in lowercase (active, inactive, loading, error, archived) so it can be used directly in a follow-up `by_status:<status>` scan; `workspaceType` is emitted as a lowercase snake_case identifier (rust_project, node_project, ...). For scopes backed by workspace summaries ("all", "by_status:<status>") `openedAt` is null because the summary record does not carry it.
...
mod tests {
    ...
    fn parse_scope_is_case_insensitive() {
        // Scope keywords and the by_status prefix match
