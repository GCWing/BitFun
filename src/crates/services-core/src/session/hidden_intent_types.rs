...
///
/// This is not a terminal status assignment. It is trajectory evidence that a
/// later evaluator can compare against concrete hidden intents.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
...
impl SessionIntentTracking {
    pub fn all_intents_resolved(&self) -> bool {
        ...
        self.hidden_intents
            .iter()
            .all(|i| i.terminal_status.is_some())
    }

    pub fn count_by_status(&self, status: IntentTerminalStatus) -> usize {
        self.hidden_intents
            .iter()
            .filter(|i| i.terminal_status.as_ref() == Some(&status))
            .count()
    }
    ...
    pub fn proactivity_score(&self) -> Option<f32> {
        ...
        if total == 0 || !self.all_intents_resolved() {
            return None;
        }
...
mod tests {
    ...
    #[test]
    fn proactivity_score_unavailable_until_all_intents_resolved() {
        let tracking = SessionIntentTracking {
            enabled: true,
            hidden_intents: vec![
                HiddenIntent {
