...
enum Command {
    ...
    /// Generate trading signals from pipeline and CSV data
    Signal {
        /// Path to pipeline YAML config
        #[arg(long, value_name = "FILE")]
        config: PathBuf,
        /// Path to CSV tick data
        #[arg(long, value_name = "FILE")]
        csv: PathBuf,
        /// Output signals JSON file (default: stdout)
        #[arg(long, value_name = "FILE")]
    ...
    /// Analyze CSV tick data and produce market report
    Analyze {
        /// Path to CSV tick data
        #[arg(long, value_name = "FILE")]
        csv: PathBuf,
    ...
    Mcp,
    /// Start ACP server (Agent Communication Protocol)
    Acp,
...
// ── Stub handlers for new subcommands (implemented in later R-IDs) ──────

fn run_signal(config_path: &PathBuf, csv_path: &PathBuf, output_path: &Option<PathBuf>) -> Result<()> {
    info!("taiji signal --config {} --csv {}", config_path.display(), csv_path.display());
...
fn run_analyze(csv_path: &PathBuf, _symbol: Option<&str>) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    // Resolve tier from TAIJI_API_KEY (defaults to Free if not set)
    let (_tier, config) = auth::resolve_tier()
        .map_err(|e| anyhow::anyhow!("Auth error: {}", e))?;
    ...
    match cli.command {
        Some(Command::Backtest {
            ...
            run_backtest(&config, &csv, parallel)
        }
        Some(Command::ReloadConfig { config }) => run_reload_config(&config),
        Some(Command::Signal { config, csv, output }) => run_signal(&config, &csv, &output),
        Some(Command::Analyze { csv, symbol }) => run_analyze(&csv, symbol.as_deref()),
        Some(Command::Mcp) => {
            let cfg = config.clone();
            mcp::run_mcp_server_sync(cfg)
        }
        Some(Command::Acp) => acp::run_acp_server(config.clone()),
        Some(Command::Auth { command }) => run_auth_command(command),
        None => {
            // Default pipeline mode (backward compat)
            let config_path = cli
                .config
