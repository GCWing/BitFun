#!/usr/bin/env node
/**
 * Probe: does the wasm `prepareInferRequest` signature depend on the request
 * body? Calls it with two different bodies and compares the signature headers.
 * Mirrors the CLI's `vVi`: prepareInferRequest(endpoint, body, model_key, source).
 */
...
const userInfo = JSON.stringify({ uid: 'test-uid-0001', encrypt_user_info: 'enc-test', key: 'key-test' });
const ctx = ctxNew(machineId, userInfo);
const endpoint = 'https://gateway.qoder.com.cn';
const bodyA = JSON.stringify({ model: 'auto', messages: [{ role: 'user', content: 'hello' }] });
const bodyB = JSON.stringify({ model: 'qwen3.8-max-preview', messages: [{ role: 'user', content: 'different body longer' }] });
const r1 = readResult(prepareInfer(ctx, endpoint, bodyA, 'auto', 'system'));
const r2 = readResult(prepareInfer(ctx, endpoint, bodyB, 'qwen3.8-max-preview', 'system'));
const sig1 = r1.headers['Authorization'] || r1.headers['authorization'];
const sig2 = r2.headers['Authorization'] || r2.headers['authorization'];
console.log(JSON.stringify({
  url1: r1.url, url2: r2.url,
  body1_head: r1.body ? r1.body.slice(0, 80) : null,
  body2_head: r2.body ? r2.body.slice(0, 80) : null,
  headerNames1: Object.keys(r1.headers).sort(),
  headerNames2: Object.keys(r2.headers).sort(),
  sig1, sig2,
  authEqual: sig1 === sig2,
  sameHeaderNames: JSON.stringify(Object.keys(r1.headers).sort()) === JSON.stringify(Object.keys(r2.headers).sort()),
}, null, 2));
