...
/// dialog turn.
///
/// Like [`PERMISSION_MODE_CONTEXT_KEY`], the owning surface resolves
/// `turn -> session -> global default` once at submission time and writes the
/// result here; delegated subagents inherit the same value.
