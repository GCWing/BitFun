...
|---|---|---|---|---|---|
| 默认低摩擦 | PRD-01、PRD-02 | 快速路径轻量、体验投影层 | 配置化策略画像、项目画像 | P0 | 无配置项目、小工具、文档改动任务回放；统计首次有用动作耗时和低风险任务完成率 |
...
| 主动配置信任 | PRD-09 | 项目规则与主动配置分层 | OpenCode 兼容层、安全边界 | P0、P2 | hook、plugin、MCP、自定义工具 fixture；验证来源、hash、权限、触发条件和信任状态 |
| Kernel 扩展点 | PRD-16 | hook、事件、打点、插件效果候选、内核权威状态 | OpenCode 兼容层、质量数据面 | P-1、P1 | 空实现、过期 epoch、重复 event、候选效果丢弃、打点采样 fixture；验证不影响默认任务 |
