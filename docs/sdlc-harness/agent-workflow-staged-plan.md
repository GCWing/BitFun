...
| 默认执行 | 先固定当前修改、明确 Git range 或 PR 的 base/head、文件状态、diff 引用和完整度；本地显式审查只做一个只读 reviewer；PR/团队路径按规则给 Review 面板和就绪度摘要 |
...
- 目标证据先于 Review 决策：当前工作区使用一次有界 `HEAD -> worktree` 取证，但没有 immutable snapshot，因此最终 evidence status 始终为 `limited`；显式 Git range 由目标准备层固定 base/head，完整且无遗漏时才可为 `complete`。Reviewer 不自行猜 ref，缺失、截断或预算耗尽必须进入覆盖说明并阻止 clean approval。
