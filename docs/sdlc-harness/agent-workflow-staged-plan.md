...
- 目标边界仍是 PR adapter 保留 provider identity、远程 diff 读取和评论发布，把 diff 映射为统一 Review 目标事实，并消费统一质量决策和结果投影，不拥有 reviewer 调度。当前仅按工作区和文件重叠启发式展示相关 Review 会话数量，没有稳定 PR 身份关联；既有 PR Review MiniApp 仍保留 fast / focused / deep 和独立 AI 草稿路径，尚未完成统一收敛。目标态保持三层：CI、审批、branch protection 与 mergeability 是平台原始权威事实和执行约束；既有变更就绪度 / PR 门禁模块基于证据、策略和人工决策生成 BitFun 就绪度；AI Review 只提供建议优先处理的问题、覆盖缺口和结果时间，不能单独阻断。稳定身份与失效规则完成前，不把启发式关联写成结果新鲜度。Verify 的权威证据生产和就绪度规则仍未在本阶段实现。

2026-07-10 合入后产品复盘：

- 统一入口、自适应强度、只读 Reviewer、独立 ReviewFixer 和同侧栏 follow-up 已形成可用基线，不再新增 Review 执行分支。
- 下一优先切片先做本次 Review 内的问题处理和诚实的“未发现问题”结论；修复后跨 Review 增量对照必须等待稳定身份、持久化所有者和失效规则明确，不能把启发式关联包装成已关闭。
- PR Review MiniApp 的独立强度和 AI 草稿路径是尚未收敛的历史产品实现；后续只能向统一 Review 结果投影收敛，不能继续扩展为第二套 Reviewer 调度。
- L2/L3 成本确认下一步应说明新增覆盖的收益，并提供“按建议审核 / 只保留核心检查 / 暂不审核”，而不是增加更多策略档位。
- PR 自动审查、反馈学习和大规模任务控制台仍是候选场景；没有稳定场景、问题身份和成本数据前，不进入当前实现承诺。
- 详细竞品证据、当前体验差距和明确非目标见 [research/review-product-experience-benchmark.md](research/review-product-experience-benchmark.md)。
