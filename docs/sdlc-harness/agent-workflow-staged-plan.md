...
- 成本增加前必须说明为什么值得，且用户可以降级、暂停或只收敛已完成部分。
...
| 成本 | L2/L3 前显示预计 token/耗时和收益，允许降级 |
...
| L3 | 严格审查 | 大型 PR、核心接口、安全敏感、团队强策略、用户明确要求 | 必须预算确认，可降级 |

原则：

- L0/L1 服务多数任务，L2/L3 只服务风险或明确意图。
- reviewer 默认只读；修复必须进入用户批准的执行阶段。
- 两轮审查没有新增有效问题时，应建议降级或停止。
...
| 解决率不足 | 先问是否追加预算或降级目标，不静默扩大 token |
