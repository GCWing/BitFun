...
- 已采纳并实现：普通与严格 Review 由具体未解决问题驱动有界专项复核；内置规则、兼容 Skills 和用户配置的只读审核能力进入同一个内部能力目录，用户仍只看到一个 Review。专项复核绑定明确问题、改动文件范围和证据预期；普通最多两个，严格最多三个且与条件质量检查共用额度，最多并发两个。远程工作区禁用尚不能保证范围隔离的自适应专项复核；历史受管文件包仍按兼容路径执行。
- 已采纳并实现：大目标文件分包与审核维度不成倍扩张；普通零专项复核路径不加载能力目录，专项复核失败不自动重试。现有日志用于离线验证 token、耗时和覆盖，不新增 Review 专用遥测平台。
