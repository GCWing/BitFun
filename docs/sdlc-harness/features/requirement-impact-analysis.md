...
需求影响分析是复杂项目和高风险生命周期场景能力，不是 P0 默认能力。它的产品价值是在用户明确处理 spec/API/发布/事故，或自适应控制判断变更可能影响多个工程资产时，给出候选影响面、置信度、证据和验证建议。

它输出的是候选集合，不是完整事实。高风险、低置信链接必须人工确认，确认或拒绝结果写回 [交付物图谱](../architecture/artifact-graph.md)，用于后续校准。

快速路径下，本模块不应打断用户；最多在摘要中提示“可能需要需求/影响确认”的下一步。
