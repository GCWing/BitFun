...
BitFun 内部以规范事件、交付物、权限和策略模型为准；OpenCode API 负责降低插件迁移成本。插件允许提供观察、建议、验证提示或证据候选；安全边界、通过/失败、强制/阻断和审计事实由 BitFun 策略层写入。

P0/P1 不承诺任意社区插件无修改运行，也不把 OpenCode 插件运行时作为默认能力。默认策略是发现、展示、禁用、信任审查和内核与产品扩展契约预留；执行型自定义工具必须等到插件适配主机和隔离策略成熟后再启用。

长期目标不在于复制 OpenCode 运行时，而在于形成一个可承载多生态适配器的受控插件适配主机：OpenCode、Kiro hook、Claude hook 或 BitFun 原生插件都通过同一 Host ABI 返回候选效果。Agent Kernel、安全控制面和 Execution 层按职责维护权威事实。该目标不进入 P0/P1 的实现关键路径。
...
- 近期优先稳定 Agent Kernel hook、事件、打点、工具复写候选、UI contribution 和插件效果候选契约。
- 插件适配主机作为后续生态能力由 Product Assembly 注册，并由 Agent Kernel 事实、安全控制面和 Execution 层约束，默认使用本地受控 IPC，不暴露公开 localhost 服务。
- 一个插件适配主机运行时允许承载多个项目执行域，但项目之间的事件、信任、权限、工具复写表和插件状态必须隔离。
...
- 运行时组件和依赖缓存允许共享；项目执行域、工具复写、权限和审计不能共享。
...
```text
Agent Kernel / Security Control Plane / Execution Layer
  权威事件事实 / 策略决策 / 安全边界 / 工具执行结果 / 审计
...
Agent Kernel 负责权威状态事实，安全控制面负责安全事实和副作用决策，Execution 层负责工具执行结果。插件适配主机负责插件语言运行时和生态 API 适配；Extension event router 负责把内核事件事实、安全事件和执行事件投递给已注册适配主机。适配主机崩溃、重启、超时或返回旧结果时，系统必须能够降级、拒绝或重放事件。插件隔离等级必须映射到 [安全边界](../architecture/security-boundary.md) 的执行与沙箱矩阵；Host 内部的 JS/TS 运行时、worker 或子进程不构成自动可信边界。

项目执行域（Project Domain）包含项目根、worktree、执行主机、远程上下文、信任记录、事件订阅、工具复写表、权限范围和审计流。一个 Host 进程允许管理多个执行域，但不得允许插件全局变量、订阅、复写表、环境变量或授权跨域复用。

默认采用一个插件适配主机监督进程。监督进程按风险启动插件单元（Plugin Cell）、worker、子进程或容器；这些都是 Host 管理的执行单元，不改变 Kernel/Host 的外部契约。
...
| worker | 建议、guard、轻量自定义工具 | 独立工作线程或等价隔离；所有副作用走 Kernel / Execution facade | `process_isolated` |
...
发现主动配置
  -> 记录来源、hash、权限和范围
  -> 分类信任状态
  -> Kernel / Security / Execution 写入规范事件事实
  -> Extension event router 按事件类型投递到插件适配主机
  -> 适配主机选择项目执行域和生态适配器
  -> 在 deadline、权限 facade 和隔离约束下执行 plugin hook
  -> 适配主机返回 PluginEffect
  -> Kernel / Security / Execution 校验 epoch、权限、安全策略和幂等键
  -> Kernel / Security / Execution 应用、降级、拒绝或记录候选效果
  -> Security Control Plane 追加审计事件
```
...
- **内核事实优先**：状态由 Agent Kernel 事实维护，权限和审计由安全控制面写入，工具执行结果由 Execution 层写入；适配主机只返回候选效果。
...
| P1 | Kernel 与产品扩展契约 v0：规范 hook、事件、打点、工具复写候选、插件效果候选和空实现 |
...
| 适配主机与 Kernel / Security / Execution 时序不一致 | envelope、deadline、epoch、幂等键和过期响应丢弃规则必须测试覆盖 |
...
- P0/P1 阶段即使没有插件适配主机，Kernel 与产品扩展契约空实现也不会影响默认任务。
