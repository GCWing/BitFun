...
- 插件或配置变化先在后台准备候选版本；来源仍启用、旧版本仍符合当前策略时，代码或依赖更新准备失败可继续
  使用上个可用版本。停用、删除、来源撤销、权限收紧或安全策略失效必须撤下旧贡献，不能为不中断评审而回退。
...
- 取消传播、有界队列、输入/输出大小限制；
- 无效响应丢弃、迟到响应不再提交；
- 有限次数恢复和重复错误聚合；仅在来源仍启用且旧版本仍合规的代码/依赖更新失败时回退；
- 凭据和敏感环境变量不进入普通日志或报告。

未知配置字段、事件、Client 方法或 Hook 参数必须局部失败。未知读接口返回稳定的不支持错误；未知写接口不执行，
也不伪造成功。任何不兼容项都不能触发无限重试、日志风暴、主界面等待或 Agent 循环卡死。
