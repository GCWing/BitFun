...
- 普通项目、临时目录、小工具、脚本、演示和文档改动：默认简洁快速。
...
| 平台 / 插件维护者 | 主动配置以待确认能力展示，确认后再参与执行 | hook、plugin、MCP、自定义工具或远程执行面变化 | 主动配置先作为未授权候选，隔离后授权 |

用户画像用于选择默认入口、解释语言和提示密度。内部策略画像不是用户侧主概念。用户看到的是任务状态、弱提示、确认、受限原因、设置项和下一步。

## 4. 核心体验路径

| 路径 | 触发 | 用户看到 | 系统动作 |
|---|---|---|---|
| 快速开发 | 低风险任务、临时目录、个人项目、探索性改动 | 任务结果、已运行检查、未验证项、下一步建议 | 轻量项目理解，保持低干预策略 |
...
| 临时项目与受管项目 | 临时项目默认低摩擦；受管项目按配置显露审查、审计、批准和门禁 |
...
| 插件 / MCP / hook 入口 | 作为能力扩展和主动执行面 | 默认最小权限；先展示来源、hash、权限、触发条件和信任状态，再允许执行 |
...
| P2 | 团队配置、路径规则、强制检查和风险接受 |
| P3 | 需求、PR、发布、事故和回归资产追溯 |
| P4 | 真实任务回放、策略评测、指标校准和保留集 |
