...
| Review 目标与 Reviewer 工具 | 当前修改和明确 Git range 由目标准备层固定可证明的 revision、文件状态、diff 引用和完整度；PR 目标由后续 PR adapter 接线。Reviewer 以有界 prepared `GetFileDiff` 为 changed-code source of truth，只有本地 head 匹配且整个工作区干净时才用现有 Read/Grep/Glob/LS 补充上下文；保留旧入口已有 Git 能力但不允许覆盖 prepared target；不新增 Git 工具或任意 shell |
