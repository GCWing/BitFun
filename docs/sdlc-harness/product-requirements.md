...
| 生态扩展预留 | 先稳定 Plugin Runtime Host contract、binding、envelope、disabled stub、Event Manifest、Tool ABI、Permission/Effect 和 UI descriptor；OpenCode 等插件生态后续接入时必须在 BitFun 边界内生效 |
...
| 扩展契约 | Plugin Runtime Host contract、生命周期 envelope、Event Manifest、Tool ABI、Permission/Effect、UI descriptor 和插件效果候选 | 先提供稳定契约；外部插件运行时不作为默认能力 |
...
| P1 | 风险、远程能力缺口、沙箱降级和推荐检查变得可理解 | 需要风险分类、提示降噪、sandbox capability 事件和扩展契约空实现；不执行外部插件运行时 |
