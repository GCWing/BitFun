...
质量数据面是事实数据层，不是质量门禁层。它负责把项目画像、任务、工具调用、文件变更、验证命令、控制决策、安全授权、审查、CI、发布和运行期反馈整理成可追踪、可裁剪、可回放的数据事实。

新的 P0 不再以 PR 门禁为中心。P0 事件集必须优先支撑三件事：

1. 普通任务结束时能解释做了什么、验证了什么、没验证什么。
2. 安全敏感动作能追溯允许、询问、拒绝、应急放行的原因和范围。
3. 自适应控制能用真实数据校准是否过度打断、误升级或漏提示。

证据包、交付物图谱、风险分类器、变更就绪度、PR 门禁和智能体评测都消费同一事实层，但不能直接读取内部存储或各自重新定义事实字段。
...
| [Codex approvals/security](https://developers.openai.com/codex/agent-approvals-security) | 审批、安全授权和工具执行要能审计，但不能把安全 prompt 做成质量门禁 |
...
- 内部事件模型保持规范；OpenTelemetry、CDEvents、SLSA 等是导出适配，不是内部事实来源。
...
- 统一任务、会话、控制、安全、工具、文件、验证、审查、门禁、成本和主动配置的事件域。
...
| `control.decided` | 固化模式、触发原因、推荐/强制动作 |
...
- **隐私分级**：区分公开、项目、敏感和凭据；凭据不进入长期事件。
- **证据引用**：大日志、报告、截图、轨迹使用 `EvidenceReference` 引用，不内嵌。
- **语义稳定**：核心字段采用稳定命名和版本，不把 UI 文案或外部载荷直接写入事件模型。
...
| 风险 | 反证或治理要求 |
|---|---|
| 遥测膨胀 | P0 事件集必须能解释快速路径和安全决策，不能追求全量 SDLC |
| 原始日志泄露 | 证据包不应包含长期原始日志；敏感片段必须脱敏 |
| 事件不可治理 | 每个事件域必须定义负责人、保留周期、结构版本和导出策略 |
| 结构漂移 | 新事件或字段必须先进入注册表，并提供兼容读取或迁移策略 |
| 模型摘要覆盖事实层 | 模型输出只能作为派生证据，不能覆盖原始命令/CI/审查事实 |
| 跨模块耦合过重 | 上游模块只能依赖查询接口和事件结构，不能直接读取内部存储 |
| 审计不可复现 | 控制、安全、就绪度、门禁、审查、风险等关键结论必须能追溯到事件 id 和证据引用 |
