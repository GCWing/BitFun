...
项目画像采用分层成熟度：快速路径先读取低成本信号；PR、团队治理、发布和复盘场景再补齐负责人、规则、验证能力、外部系统和主动配置状态。这样既能降低误判和脏链接，也能保证第一次有用动作足够快。
...
- 主动配置发现只记录候选执行面，信任审批由安全边界和团队策略完成。
