...
| P2 | 准备或审查 PR 时，用户先获得目标正确的统一 Review，再获得可复制协作摘要和规则说明 | Review 目标证据、条件式 PR 入口收敛、PR 就绪度、团队规则折叠说明、风险接受 | workspace/Git target evidence、只读证据状态、PR 视图、规则来源合成、风险接受记录 | 当前只闭合目标正确；provider 入口必须由上线数据重新证明；强制项只在配置或确定性风险下出现 |
...
| P2 | Review 目标证据、条件式 PR 单入口、PR 就绪度、团队规则来源、路径规则、风险接受 | PR 或团队场景可准确审查目标并获得可复制摘要和规则说明；普通任务不被团队流程打断 | 审错 base/head；dirty workspace 污染；prepared diff 越界或超限；证据不足却批准；团队规则冲突；PR 摘要过重 | workspace/range 目标与失效用例、有界 diff、聚合预算、越界、fail-closed 报告、规则来源和风险接受测试；provider fixture/head stale 仅在后续立项时添加 | 目标证据使用明确状态；PR 视图按需生成；不自动 checkout；不创建默认强制检查；上线后按指标决定是否继续 |
...
用户准备或审查 PR、分享变更或命中团队规则时，BitFun 先确认当前工作区、明确 Git range 或 PR 的真实 base/head 和 diff 完整度，再提供统一 Review、可复制协作摘要、规则来源和风险接受入口，而不影响普通任务中间过程。
...
| PR 只进入一套 Review | PR adapter 固定 provider identity 和目标证据，PR MiniApp 启动统一 Review，平台事实、BitFun 就绪度和 AI 建议分层 | 不保留 fast/focused/deep 或独立 AI 草稿；不做自动 Review、同 diff 复用或新 provider |
...
- Reviewer 不新增 Git 命令面，也不删除旧入口已有能力；prepared target 只消费禁用 external diff/textconv 的有界 exact diff。Review 分页使用 opaque cursor，并按 reviewer 限制为 240,000 返回字符或 16 个不同页面；重复页面不重复计费，超限返回结构化 limited evidence。rename 强制检测并全链路保留 old/new path。未跟踪内容取证受文件数和总字节聚合预算约束，单文件限制低于一页并拒绝 symlink/reparse point。本地 head 不匹配或整个工作区不干净时，不使用 live repository context。
- 显式 Git range 只有在证据完整且无遗漏时可报告 `complete`；mutable workspace 始终为 `limited`。`limited`、`stale`、`failed` 以及无效 summary 都不能输出 clean approval。
- 启动消息结果不确定时保留并打开已创建的 Review 子会话，不删除可能已开始执行的 backend session。
- PR 摘要减少审查人追问，但不默认阻塞。
...
| 必要实现 | 行为轻量的 target evidence contract；workspace/Git range 一次取证；准确文件状态与完整度；跨 Review 子会话的目标上下文传播；独立 evidence status；prepared target/report fail-close；opaque cursor 与 reviewer 聚合预算；`matching_clean` 时保留普通 Read/Grep/Glob/LS 上下文 |
| 兼容 | 现有工作区 Review 保持默认路径；历史 manifest 无 target evidence 时明确走 legacy workspace fallback，不伪装成精确 range/PR |
| 风险前置 | 删除、重命名、二进制和超大文件；dirty workspace 污染；prepare 后工作区变化；remote backend 无有界 exact diff；文件名 prompt injection；Git 外部 diff/textconv 副作用 |
...
该项不是已承诺 PR。只有当前改动上线后，指标证明 provider 目标错误或重复入口仍是高频决策障碍，且可以复用现有 Review 流程而不新增调度/缓存/生命周期系统时，才重新立项。

| 项 | 候选边界 |
|---|---|
| 用户收益 | 在现有 PR 面板发起统一 Review，并看懂平台状态、BitFun 就绪度和 AI 建议的区别 |
| 必要实现 | PR adapter 固定 provider identity/base/head/diff；PR MiniApp 调用统一 Review；移除 fast/focused/deep 与独立 AI draft 路径；head 失效；打开原 Review；认证、空态、错误和 reduced coverage i18n |
| 保留 | provider 发现、PR identity、CI/审批/mergeability、URL、手工 composer、确认发布和审计生命周期 |
| 风险前置 | head 在 Review 期间变化；provider diff 截断/分页；旧草稿迁移；平台事实被 AI 文案覆盖；认证失败；PR 面板状态与 Review child 恢复不一致 |
| 验证 | ReviewPlatform provider fixtures；MiniApp bridge/asset tests；ReviewService 与 session recovery focused tests；head stale tests；i18n audit；Web type-check |
| 回退 | 统一 Review 不可用时保留 PR 浏览和手工 composer，明确提示暂不可 AI Review；不得恢复第二套 AI 决策路径 |

当前 PR 后立即停止实现并评审真实数据。不预先承诺 provider 接线、“不处理”、追问、finding 生命周期、自动评论、增量缓存或新的结果状态。

### 8.5 明确延期

- 自动或 ready-after-push Review、去抖和持续审查。
- 跨 Review `finding_id`、问题关闭、同 diff 复用和反馈学习。
- 自动 clone/fetch/checkout/worktree、完整远程仓库缓存和跨仓上下文。
- Reviewer 任意命令执行、测试运行或 Verify evidence producer。
- 新 provider、组织分析后台、大规模 Review 控制台和新的设置/规则 DSL。
- Review 结果新增“不处理”、上下文追问、跨 Review 状态或新的 finding 生命周期；是否需要其中一个动作必须由上线数据重新证明。
