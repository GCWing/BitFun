...
| 必要实现 | 行为轻量的 target evidence contract；workspace/Git range 一次取证；provider PR 固定 identity/base/head 并按文件读取 diff；准确文件状态与完整度；精确 PR/revision 结果投影；跨 Review 子会话的目标上下文传播；独立 evidence status；prepared target/report fail-close；opaque cursor 与父 Review turn 聚合预算；仅 `matching_clean` Git range 保留普通 Read/Grep/Glob/LS 上下文 |
