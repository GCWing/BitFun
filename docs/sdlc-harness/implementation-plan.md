...
| 扩展与插件 | 预留 Plugin Runtime Host contract、Event Manifest、Tool ABI 和候选契约 | 空实现、过期候选和幂等校验 | 工具复写契约、UI descriptor 和主动配置信任 | 插件运行时主机原型、生态评测 |
...
| P1 | 风险、远程和不可支持状态变得可理解，用户知道该补什么验证 | 弱提示、推荐检查、远程/沙箱降级、Plugin Runtime Host contract / Event Manifest v0、证据引用摘要 | 风险分类器、提示降噪、sandbox capability 事件、扩展契约空实现 | 高风险解释清楚；低风险仍快速；远程和不可支持状态有替代路径 |
...
| P1 | 风险提示、推荐检查、远程不可支持状态、沙箱降级说明、Plugin Runtime Host contract / Event Manifest 空实现、证据引用摘要 | 用户能理解风险和能力缺口，并知道补什么验证 | 风险提示泛化成噪音；推荐检查不可运行；远程缺口被本地验证掩盖；扩展契约空实现被误认为可用插件能力；证据引用缺少新鲜度 | 中/高风险用例必须说明原因、证据、建议和跳过后果；推荐检查要有不可运行分支；扩展候选不能写权威状态 | 风险分类和提示生成后台化；同类提示合并；扩展契约有 deadline、epoch、幂等键；证据引用只保存摘要和引用 |
...
| 插件和工具复写越权 | 外部插件、OpenCode 适配层或工具复写候选直接影响权威状态 | 执行链不可信，后续需要重构 Plugin Runtime Host、Tool ABI 和 Security Bridge 边界 | 早期只保留 Plugin Runtime Host 空实现和候选契约；候选不能写通过、失败、阻断、审计或授权状态 |
...
| 执行与沙箱矩阵 | 本地、远程、ACP、MCP、MiniApp/前端、插件运行时主机、浏览器/桌面、云端任务的执行位置、隔离等级、降级原因和用户选项 | `allow_in_sandbox` 只在真实隔离可用时出现 |
| 扩展契约 | Plugin Runtime Host contract、Event Manifest、Tool ABI、UI descriptor、工具复写候选、插件效果候选、deadline、epoch 和幂等键 | 未来适配器可接入，空实现不影响默认任务 |
...
| 扩展契约可预留但不干扰 | Plugin Runtime Host contract、Event Manifest、Tool ABI、打点、工具复写候选契约和空实现 | 不执行 OpenCode 插件，不让候选写权威状态 |
