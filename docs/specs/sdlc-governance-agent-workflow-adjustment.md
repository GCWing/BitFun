...
- [agent-workflow-staged-plan.md](../plans/sdlc-governance-agent-workflow-staged-plan.md)：将本文需求压回低风险任务、提交前审查、CI/测试失败、PR 评论批量修复和大规模迁移/审计等真实用户场景；不新增独立阶段路线。
...
权威边界：本文是候选产品调整提案，不替代 [product-requirements.md](sdlc-governance-product-requirements.md)、[implementation-plan.md](../plans/sdlc-governance-implementation-plan.md)、[governance/metrics-spec.md](sdlc-governance-metrics-spec.md) 或 QDP 事件注册表。任何条目被采纳前，必须回填到对应权威文档；未回填前不得作为正式 PRD、阶段承诺、门禁规则或指标口径执行。
...
统一 Review 入口、只读审查与修复分离已经成为当前基线，不再重复列为候选。下表只保留尚未采纳的议题，不是正式 PRD 编号。采纳任一议题前，必须合并到 [product-requirements.md](sdlc-governance-product-requirements.md)，并按需要同步 [implementation-plan.md](../plans/sdlc-governance-implementation-plan.md)、[governance/metrics-spec.md](sdlc-governance-metrics-spec.md) 和 QDP 事件注册表。
