...
- 可移植引擎（settings 解析、payload 构造、进程执行、决策合并）位于
  `bitfun-agent-runtime::native_hooks`。`bitfun-core::native_hooks` 负责配置发现、
  开关与按事件分发辅助；分发点应调用这些辅助，而不是内联执行 hooks。
- 三个不同概念共用 “hook” 一词：这些原生用户 hooks、内部编译进仓的
  `post_call_hooks`，以及其他 AI 应用的只读外部 hook 目录（`external_hooks`）。
  必须分开，不要混用。
