...
flowchart LR
  ...
  SourceService["Runtime Configuration / 来源服务"]
  Snapshot["自动生成的执行版本记录"]
...
权限存储。
Runtime Product Assembly 只选择并注入 `PluginRuntimeBinding`、执行服务和产品能力/策略上限；动态来源发现、
依赖准备和执行版本记录由 Runtime Configuration/来源服务直接驱动 Host，插件文件变化不能成为产品组装的隐式输入。

### 2.1 开发视图

| 开发部分 | 负责 | 不能承担 |
|---|---|---|
| 兼容来源准备 | 读取配置适配器提供的插件、工具、顺序和依赖，生成当前执行版本记录 | 要求作者维护 BitFun 专用包或在界面线程安装依赖 |
| 依赖准备服务 | 按兼容版本管理 npm 配置、Arborist、package-lock、缓存和安装锁 | 执行插件代码或擅自启用安装脚本 |
| 脚本执行服务 | 管理固定版本 Bun、每 target 进程、健康和资源回收 | 理解 OpenCode Hook 语义或提交 BitFun 业务状态 |
...
| Plugin Runtime Host | 期限、取消、有界队列、进程失联、响应校验和诊断 | 决定 OpenCode 加载顺序、权限结果或工具结果 |
...
OpenCode 项目和用户目录可以直接成为兼容来源。BitFun 自动生成当前执行版本记录，但不要求作者
手工构造 `.bitfun/plugins`：
...
默认流程自动生成和更新记录，不新增多轮人工确认。内容变化使旧执行进程失效并后台准备候选版本；新进程
健康检查通过后原子切换。准备失败时保留旧进程和清晰的 stale 状态，不让界面等待依赖安装。

执行版本记录是 BitFun 的内部缓存和诊断信息，不是新的插件包规范，也不要求所有动态 import 在启动前形成完整
依赖清单。本地开发插件缺少 package-lock 或完整性元数据时仍可运行，只使用更保守的目录摘要和重载策略；运行时
发现新依赖后更新记录。不能为了追求完全可复现而拒绝 OpenCode 本身能够加载的合法插件。
...
- `ready`：执行进程健康，真实贡献已加载。
- `active`：贡献已经进入对应归属模块。
- `degraded`：部分接口、策略或平台能力不可用，其他贡献仍可用。
- `restarting`：内容变化或 worker 丢失后重建。
- `failed/paused`：无法安全继续；保留诊断和用户恢复入口。

来源、启用、依赖准备和运行状态是不同事实，但默认兼容流程不要求逐层重复确认。OpenCode 标准目录和配置来源
按其默认自动加载。

本地文件或配置 spec 变化时自动准备候选进程；健康检查通过后在无在途调用的边界切换。来源仍启用且旧版本仍
满足当前策略时，代码或依赖更新准备失败可以继续使用上个可用版本。显式停用、删除、来源撤销、权限收紧或
安全策略失效必须先阻止新调用并撤下旧贡献，不能走旧版本回退。软件包的显式“安装”和“更新”分开：安装增加
来源并启用，更新先显示工具、Hook、依赖和权限变化，再切换候选版本；停用和回退无需删除缓存。
