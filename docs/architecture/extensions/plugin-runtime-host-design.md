...
| 插件运行时主机 | 调用路由、期限、取消、队列、健康、响应校验和故障状态 | OpenCode 语义、最终业务状态、产品入口接口 |
| 生态适配器 | 保留生态加载顺序、参数、结果、错误和生命周期语义 | 成为新的 BitFun 业务归属模块 |
| 依赖准备服务 | 按生态兼容版本准备依赖、缓存和安装锁 | 执行插件代码或决定生态加载语义 |
| 脚本执行服务 | 准备脚本运行时和每插件 target 进程，执行类型化请求并回收故障进程 | 决定工具权限、修改会话或直接操作界面 |
...
| `src/crates/assembly/core` 的产品组装点 | 当前选择插件运行时 binding 与可用性 | 只注入窄 `PluginRuntimeBinding`、执行服务和产品能力/策略上限；不发现动态来源、不准备依赖、不加载适配器 |
...
| `src/crates/adapters/opencode-adapter` | 当前静态解释少量 OpenCode 文件 | 承担 OpenCode 格式和进程协议适配；不持有最终工具、配置、权限或界面状态 |
...
trait object 并注入 Host 的位置。Host 只围绕注入的 adapter 工作，不加载具体生态模块；产品组装只选择已编译的
adapter capability/factory 和能力上限。Runtime Configuration/来源服务随后通过窄 binding 驱动动态来源与执行
版本，来源变化不触发产品重新组装。
...
依赖准备、模块导入和健康检查在后台执行，不阻塞桌面或 TUI 主线程。来源仍启用、旧版本仍满足当前策略时，
代码或依赖更新准备失败可以继续使用上一个可用版本并标记更新失败；首次准备失败只影响相应插件。显式停用、
...
- worker 崩溃后由执行服务重建；同一插件连续失败时只暂停相应 target 或贡献，并提供恢复入口。
...
- Host 必须持有完整进程树：Windows 使用 Job Object，Unix 至少使用独立 process group；取消、超时、停用和退出都终止整棵树，不能只回收直接子进程。
...
来源记录、用户启用、依赖准备和运行状态是不同事实，但默认流程不要求用户逐层重复确认。
