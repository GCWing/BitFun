...
| 方式 | 用途 | 行为 |
|---|---|---|
| 兼容来源 | 默认打开已有 OpenCode 项目 | 启动时发现、合并并生成只读兼容结果；源文件变化后重新解析。BitFun 不写回原文件。 |
| 显式导入 | 用户希望改用 BitFun 原生配置 | 先显示“可直接使用、需转换、会降级”三类差异；用户选择字段后原子写入 BitFun 配置，并保留来源与撤销记录。 |

兼容来源不是 BitFun 内部权威模型，但它是合法运行输入。适配器把有效值映射到对应归属模块；归属模块仍负责
最终持久化、运行时状态和错误语义。

导入成功后，已选字段以 BitFun 原生配置为准，不再同时应用原 OpenCode 值；原来源继续被观察，变化时提示“来源已变化，是否重新导入”。未选择字段仍按兼容来源生效。这样避免同一字段被重复应用，也不会在用户不知情时覆盖已导入配置。
...
| 配置项 | 适配方式 | 明确边界 |
|---|---|---|
| 独立 TUI 配置 | 按独立来源顺序处理 `$schema/theme/keybinds/plugin/plugin_enabled/leader_timeout/attention/prompt/scroll_speed/scroll_acceleration/diff_style/mouse` | 主配置来源图和 Product TUI Blueprint 不参与其运行时优先级。 |
