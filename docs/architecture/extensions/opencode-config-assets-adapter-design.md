...
| Commands | JSON/Markdown、`$ARGUMENTS`、位置参数、`@file`、`!shell`、agent/model/variant/subtask | **已有行为、边界未抽取**：从现有 CLI/TUI 命令路径增量提取模板与执行接口 | 发现并预览模板；首次执行型启用确认后保留 OpenCode 展开顺序和子任务语义 | 文件或 shell 展开失败只终止本次命令。 |
