...
| 信任 | 产品源码、发布流程、签名和内容摘要 | 用户/工作区来源审核与独立 Host trust |
...
|---|---|---|
| 来源 | 产品只读 bundle/source root | 用户数据目录或项目 `.bitfun/plugins` |
| 锁定 | Resolved Manifest 固定 id/version/hash/signer | 运行时来源标识和用户/工作区审核状态 |
| 信任 | 产品发布与签名链确认来源；不读取 `SourceApproved` | Source approval 与首次激活的独立 Host trust |
| 生命周期 | 随产品安装、升级、回滚和撤销 | 独立安装、启用、更新、禁用和卸载 |
| 冲突 | 用户/工作区同 ID 包不能 shadow；冲突明确失败 | 用户/工作区优先级只在 Runtime Plugin 范围内生效 |
| 必要性 | required/optional 只描述缺失或隔离时的产品失败语义，不表示用户安装权 | 用户或组织可按策略管理 |
| 执行 | 仍经过 Host ABI、隔离、资源预算、权限、审计和 quarantine | 同一安全执行路径 |

产品来源信任不等于副作用授权。内置扩展执行文件、网络、shell、MCP、browser/desktop 或 remote 动作时仍必须
...
策略声明，且不得复用 Runtime Plugin 的安装状态、`SourceApproved` 或 Host trust。
...
- 远端实际能力是本地 product ceiling、远端声明能力和两端策略的交集；缺失能力进入版本化 Availability，
  不得静默回本机执行。
- 插件来源、Host trust、数据根和执行授权以执行域为准；本地信任不推导为远端信任。
