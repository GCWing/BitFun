...
本文描述 BitFun 从 `bitfun-core` 拆解阶段沉淀出的稳定产品运行时架构：先说明设计建立时的初始状态，
再定义目标状态的逻辑分层、接口与实现分离、扩展方式、安全边界和风险约束。本文只描述架构设计本身，
不记录阶段进度；执行计划仍见
[`../plans/core-decomposition-plan.md`](../plans/core-decomposition-plan.md)，详细接口和 crate 内部设计见
[`agent-runtime-services-design.md`](agent-runtime-services-design.md)，插件运行时主机、生态兼容适配层和主进程通信边界见
...
- **Product Assembly** 是 composition root，按 Desktop、CLI、Web、Server、Remote、ACP、SDK 等形态选择 feature、
...
- **产品表面分化，能力契约收敛**：Desktop、CLI、Web、Server、Remote、ACP 和 SDK 可以保留各自的交互、命令和展示方式；
  可共享的是稳定 facts、ports、artifact references、permission facts 和 capability/effect contract。
...
产品表面是 Desktop UI、CLI TUI、Web、Server route、Remote、ACP、SDK host 或其他入口中的交互适配层。它们可以围绕同一能力
提供不同命令、快捷键、布局、文案、确认方式和降级提示，但不应把这些表现细节下沉到 Agent Kernel、Execution、
Stable Contracts 或平台 provider 中。

应共享的是跨表面稳定事实：

- session / thread / workspace / execution domain identity。
- runtime event、permission fact、artifact ref、capability/effect declaration 和 unsupported/unavailable error。
- 可序列化 request / response DTO、port trait 和 UI contribution descriptor。

不应共享的是表面实现：

- Desktop pane、React component、Tauri state、window handle、快捷键和视觉布局。
- CLI TUI rendering、terminal key binding、command palette 展示和文本交互节奏。
- Remote/IM card、ACP protocol method、installer/mobile 专属投影。

CLI/Desktop parity 不能作为把 presentation dependency 放入 contract crate 的理由。Product Feature 可以声明能力、
命令映射和 UI contribution descriptor；具体渲染、命令入口和用户交互由对应 Product Surface 或 host adapter 完成。

### 2.2 ACP 边界判定

ACP 在 BitFun 中同时出现在协议入口、外部 agent bridge 和执行/权限域中。三者必须分开判断，避免把协议 surface、
生态扩展和安全执行位置混成一个 owner。

| ACP 角色 | 主要 owner | 输入 / 输出 | 禁止事项 |
|---|---|---|---|
| ACP protocol surface | `src/crates/interfaces/acp` 和产品入口 adapter | ACP wire request / response、client lifecycle、startup probing、workspace surface selection | 不拥有 Agent Kernel 状态机，不绕过 Product Assembly 选择能力 |
| External agent / tool bridge | Extension Layer / ACP bridge adapter | external agent descriptor、external tool capability、event / permission bridge descriptor | 不直接写 kernel 权威状态、permission decision、audit result 或 UI implementation |
| Execution domain / permission bridge | Stable Contracts and Security Control Plane + injected policy | execution domain、permission facts、sandbox / audit facts、unsupported/unavailable reason | ACP allow/ask 只能表达授权事实；文件、shell、网络和凭据仍按 BitFun 安全边界判定 |

### 2.3 SurfaceContract 与 DeliveryProfile

`SurfaceContract` 是跨产品表面的稳定声明，不是 UI framework。它至少包含：

| 字段 | 含义 |
|---|---|
| `surface_kind` | 稳定入口身份，例如 `Desktop`、`Cli`、`Web`、`Server`、`Remote`、`Acp`、`Sdk` |
| `delivery_profile` | Product Assembly 选择的产品形态、feature bundle、provider set 和默认策略 |
| `host_or_protocol` | Tauri / TUI / browser / HTTP route / remote control / ACP protocol / embedded SDK host |
| `execution_domain` | local、worktree、remote SSH、remote connect、ACP client、server/cloud-like 或 caller-provided domain |
| `product_core_dependency_mode` | 是否经由完整兼容 facade、只经由 API projection、只经由 Kernel API，或只消费稳定 contract |
| `capability_availability` | 每个 capability 的 full、artifact-only、status-only、temporarily-unavailable、unsupported 或 policy-denied 状态 |
| `permission_projection` | 用户可见确认方式、source surface/thread/subagent 标签和最终 decision 写入 owner |
| `artifact_projection` | full UI、read-only artifact、status card、callback URI、protocol artifact ref 或 no-UI API 结果 |
| `unavailable_reason` | 可序列化 reason code、用户可见文案 key、审计事件和远程同步状态 |
| `verification_owner` | 负责证明该表面没有能力漂移的 focused check、contract test 或手工验收记录 |

`DeliveryProfile` 由 Product Assembly 消费 `SurfaceContract` 后生成；`SurfaceKind` 只标识入口身份；`CapabilitySet`
描述运行时可用能力；`ProductCoreDependencyMode` 描述迁移期是否还需要 `bitfun-core` 兼容 facade。四者不得互相替代。

| SurfaceKind | 典型 profile / host | ProductCoreDependencyMode | CapabilitySet 来源 | 权限与 artifact 投影 | 验证 owner |
|---|---|---|---|---|---|
| `Desktop` | desktop full / Tauri + React | 迁移期可经完整兼容 facade，目标态由 Product Assembly 注入 typed parts | profile-scoped capability plan | rich approval、pane/artifact UI、source thread label | desktop focused checks、product shape tests |
| `Cli` | CLI full / TUI | 迁移期可经完整兼容 facade，目标态消费 Kernel API 与 tool/harness ports | profile-scoped capability plan | compact approval、text artifact、terminal-safe fallback | CLI focused checks、DTO round-trip |
| `Web` | web projection / browser route | API projection 或 contract-only；不直接依赖 core concrete | browser-safe capability subset | read-only artifact、browser permission projection、unsupported reason | web type/test checks、artifact projection tests |
| `Server` | server/API/relay route | API adapter 或 contract-only；不等同 Remote 或 ACP | route-scoped capability plan | callback/status API、artifact URI、audit event | server route contract tests、capability matrix check |
| `Remote` | remote control / IM projection | remote port projection；不读取 desktop UI state | remote-safe capability plan | remote approval/status card、remote sync reason | remote-connect focused checks |
| `Acp` | ACP protocol surface / bridge | protocol bridge；角色按 2.2 拆分 | protocol-safe capability declaration | ACP ask/allow/reject 映射为 BitFun permission facts，artifact ref 走协议结果 | ACP permission/external tool focused checks |
| `Sdk` | embedded runner / caller host | Kernel API 或 contract-only；调用方提供 provider/permission ports | caller-declared capability plan | caller-supplied permission port、structured artifact API | SDK fake-provider smoke、serialization tests |

Mobile Web 和 Installer 当前应映射到最接近的 `Web` / `Desktop` contract，并在需要独立能力或独立发布节奏时再提升为新的
`SurfaceKind`。

### 2.4 跨表面降级状态

跨表面能力不要求 UI parity，但必须有一致的降级语义。每个 capability 在每个 surface 上只能落入下列状态之一：

| 状态 | 用户可见行为 | 必须记录的事实 |
|---|---|---|
| `full` | surface 提供完整交互或 API 能力 | capability id、surface、execution domain、permission source |
| `artifact-only` | 只能查看或返回 artifact，不能执行完整交互 | artifact ref、read-only reason、producer surface |
| `status-only` | 只显示状态、进度、远程同步或回调结果 | status ref、last sync time、unavailable next action |
| `temporarily-unavailable` | 当前环境缺少 provider、连接或资源，未来可恢复 | reason code、retry/sync hint、audit event |
| `unsupported` | 该 surface 设计上不支持该能力 | stable unsupported reason、替代入口或 no-op proof |
| `policy-denied` | 组织策略、安全策略或凭据保护拒绝 | security decision id、policy owner、用户确认不可绕过 |

验收口径：UI/CLI/协议/API 只投影这些状态，不自行发明状态；permission prompt 必须带 source surface、thread 和 subagent
身份；artifact fallback 必须可追踪 producer 和 execution domain；Remote/Server/ACP 的同步状态必须可审计；每个
SurfaceContract 变更都要补最小 product-shape check 或手工验收记录。
...
- 安全边界散落在 SDLC Harness、tool permission、MCP、hook、shell、网络和平台执行路径中，缺少贯穿内核、
  执行、扩展和 UI 投影的一致控制面。
- `product-full` 是兼容入口当前完整能力集合的保护开关，不是未来按产品形态和 feature bundle 拆分能力的唯一事实来源。
- Agent Runtime SDK 仍缺少可对外稳定承诺的统一 facade、事件流协议、provider 注册边界、UI/API 扩展契约和
  最小依赖构建形态。

## 4. 对照分析

本节只提炼对 BitFun 分层有用的架构信号，不把其他项目的实现形态直接复制到 BitFun。外部链接和产品表面
调研证据集中在 `docs/sdlc-harness/research/`，本节只保留已经归纳进目标架构的结论。
...
```mermaid
flowchart TB
  Surface["产品入口（Product Surfaces）<br/>Desktop / CLI / Web / Server / ACP / Remote / SDK / UI host"]
...
### 6.1 产品表面与接口层（Product Surfaces and Interfaces）

放置 Desktop、CLI、Server、Remote、ACP、Web、Mobile Web、Installer、E2E 和 SDK host 入口相关代码。该层负责
产品交互、协议入口、UI/CLI/remote 投影、host adapter 和 surface command 映射。它可以把用户入口、协议请求或
命令映射为 Product Assembly API、Product Feature request 或稳定 descriptor，但不负责选择完整 capability matrix
或创建 concrete provider。

当前主要路径：`src/apps/*`、`src/web-ui`、`src/mobile-web`、`BitFun-Installer`、`tests/e2e`、
`src/crates/interfaces`。
...
- surface-local copy、shortcut、layout、permission projection 和 unsupported/unavailable 展示。

不应该放这里：

- Agent session/turn/scheduler 的状态机。
- Tool、MCP、filesystem、terminal、remote、AI provider 的通用实现。
- Product Assembly 的完整 capability matrix、feature bundle 选择或 concrete provider 注册。
- 可被 SDK 独立复用的内核 API 语义。

### 6.2 产品组装层（Product Assembly）

放置 composition root、兼容 facade 接线、产品能力选择和 provider 注册。该层按 `DeliveryProfile`、feature bundle、
capability matrix、service availability、UI contribution、extension host 和 platform provider 把下层能力组装成
产品可用 runtime parts。

当前主要路径：`src/crates/assembly/core`、`src/crates/assembly/product-capabilities`，以及迁移期 `bitfun-core`
兼容组装路径。

应该放这里：

- 产品形态选择、feature bundle 选择、capability matrix、provider 注册和 unsupported/unavailable 映射。
- `product-full` 兼容入口、profile-scoped capability plan、runtime services / tool / harness / extension host 组装。
- 把 surface command、feature pack、kernel、execution、extension 和 platform provider 装配为 typed runtime parts。

不应该放这里：

- Agent session/turn/scheduler 的状态机或产品 UI 组件实现。
- Tool、MCP、filesystem、terminal、remote、AI provider 的通用 concrete 实现。
- 运行时热路径中的无类型 service locator 或全局 mutable app state。

### 6.3 产品特性层（Product Feature Layer）
...
### 6.7 跨平台适配层（Cross-platform Adapter Layer）
...
### 6.8 稳定契约与安全控制面（Stable Contracts and Security Control Plane）
...
当前主要代码路径：`src/crates/contracts/*`，以及下层 crate 中的 provider-neutral contract。`docs/sdlc-harness/*`
提供安全、治理和质量语义来源；代码层稳定契约必须与这些文档保持一致，但文档目录本身不是 contract owner。
...
| 多产品形态能力漂移 | capability matrix、unsupported/unavailable contract、Desktop/CLI/Web/Server/SDK/ACP/Remote 验证矩阵同步维护 |
...
- Desktop、CLI、Web、Server、ACP、Remote 和独立 SDK 都通过 Product Assembly 显式选择能力，不通过下层 `if desktop/cli/web/server` 分支表达差异。
