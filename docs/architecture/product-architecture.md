...
- 插件运行时主机只负责类型化调用、期限、取消、有界队列、进程健康、响应校验和故障恢复。
...
|---|---|---|
| Desktop / product-full | 可组合 BitFun 专用包的来源、启用状态和静态工具名称预览 | 当前没有插件管理界面，也不把预览名称显示为已注册或可执行工具 |
| CLI | 当前只为 BitFun 原生包提供来源审核、启用预览、精确内容确认和停用 | 不执行插件代码、不注册最终工具；`doctor` 对完整性错误返回失败；该生命周期不套用到 OpenCode 标准来源 |
| ACP | 只读取插件状态、不可用原因和诊断 | 不把插件失败解释为 agent 失败，不接入 P0 副作用闭环 |
| Server / Remote | 只读取状态；远端运行时未完成时明确显示暂不可用或不支持 | 不在本地替远端项目启动 JS/TS；目标执行路径必须部署在实际工作区所在执行域 |
| Web / Mobile Web | 只消费后端能力服务接口和只读视图 | 不持有插件执行单元，不直接加载插件代码 |
| SDK | 默认只提供未启用状态或测试替身 | 不牵引 `product-full`、具体服务管理器或插件 host ABI |
