...
| Peer / Server | Peer Host 执行真实工作区操作；当前 HTTP Server 使用 `product-full` 组装 Embedded Runtime，并通过 `/ws` 暴露 App Server | 控制端不替远端发现或执行；Server 仅限 loopback 单用户，缺少连接级安全绑定时不扩展到远程/多用户；SSH Remote 未接入时返回不支持 |
