...
ownership 和生命周期治理；Model、Skill、Subagent 管理暂由 Shared CLI adapter 的本地 compatibility provider 保留，不属于 v18 或公开 SDK 合同。MCP 进程属于 Shared Runtime Host；controller 不启动重复 service，Shared TUI 的 MCP 管理当前返回 typed unsupported，并要求重启 Shared Runtime 后生效。GUI、Headless CLI、ACP、SDK Host、Server/Remote 仍没有 Shared consumer。该图中的多入口逻辑复用是
