...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `src/web-ui/src` | 529 | 337 | 0 | 0 | 0 | 14 | 主应用完整主题、专用 palette、widget payload 和 editor/terminal/Mermaid 域；UI exception、syntax、language identity 与 boundary fallback 已收敛到小型语义 palette，未读取的 git 同义 runtime/static key、未消费 legacy mixin、文件图标一扩展一色 root key 和局部预览派生 key 已退役。 |
...
| static root contract key | 508 |
| static root contract external usage key | 493 |
| static root contract internal-only key | 15 |
| static root contract low external usage key | 286 |
...
| `cssVarDefinitions.staticContractDefinedUnique` | 508 | 508 | 控制静态 root contract key 总量，避免新增主题时需要维护不可扩展的大型 CSS var 表 |
| `cssVarDefinitions.staticContractExternalUsageUnique` | 493 | 493 | 跟踪真正被 root 外消费的 static contract key，防止删除 key 后遗漏调用点；generated widget payload 暴露给 iframe 的 key 也按外部消费计数 |
| `cssVarDefinitions.staticContractInternalOnlyUnique` | 15 | 15 | 暴露仅定义或内部派生的 root key；剩余项必须能解释为 dynamic family 或静态效果族 |
| `cssVarDefinitions.staticContractLowExternalUsageUnique` | 286 | 286 | 暴露低外部消费 key，作为后续继续压缩 root contract 的候选队列 |
