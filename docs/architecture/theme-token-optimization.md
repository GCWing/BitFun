...
| generated widget payload key | 276 | widget iframe 对外主题变量 allowlist，作为外部边界单独预算，不计入内部 alias 读取 |
| generated widget payload compatibility alias | 14 | payload 仍暴露已登记 legacy alias，防止已生成或第三方内容读取失败 |
| generated widget payload compatibility family key | 17 | payload 仍暴露 `--radius-*`、`--spacing-*` 具体 key，同时暴露 canonical family |
| generated widget payload external-only compatibility key | 31 | payload 中仍保留且内部产品代码不再读取的 legacy key，是后续外部兼容评估和收缩队列 |
...
| 区域 | 当前出现次数 | 当前唯一色数 | 说明 |
| --- | ---: | ---: | --- |
| Theme presets | 229 | 204 | 主题个性与 palette 映射，不作为普通 app literal 直接合并；仅在同一主题内角色可证明等价时由 preset helper 复用 |
...
| --- | ---: | ---: | --- |
| Theme presets | 2 | 185 | 优先处理同一 theme 内 hex/rgb 完全等价和同 alpha 阶重复；保留主题个性 |
...
| `colorDomainNearPairs.indistinguishableTotal` | 2 | 2 | 控制专用域肉眼不可区分 pair 不继续增长，后续只能逐步降低或补充证据 |
...
| `generatedWidgetPayload.varUnique` | 276 | 276 | 控制 widget 对外主题 payload allowlist 不继续膨胀 |
| `generatedWidgetPayload.compatibilityAliasUnique` | 14 | 14 | 控制 payload 中显式 legacy alias 数量，后续只能降低或经复审调整 |
| `generatedWidgetPayload.compatibilityAliasFamilyUnique` | 17 | 17 | 控制 payload 中 legacy size family 具体 key 数量 |
| `generatedWidgetPayload.externalOnlyCompatibilityUnique` | 31 | 31 | 标记 payload 中内部代码已不读取、仅因外部兼容保留的收缩候选 |
...
1. 继续收缩 generated widget 外部兼容面：当前 payload 已移除无内部读取证据且 canonical
   已导出的低风险 legacy key；剩余 `background/bg/text/radius/spacing` 兼容名更可能被历史生成内容读取，
   删除前需要外部内容影响评估。
2. 专用域近似色复核：优先处理同一 theme、同一专用域、同一语义且用户不可区分的 near pair；
   Monaco、Mermaid、terminal、syntax 的相邻状态色必须保留或提供截图证据后再合并。
3. 自定义主题扩展硬化：把 custom theme 校验、import/export 和 preview 回退都绑定到 TS schema；
...
- generated widget payload 已停止导出低风险的 `--color-accent*` legacy、`--color-primary*`、
  ...
  旧 border 细分别名以及部分旧 font/motion 拼写；根 token 与 runtime 注入仍保留这些兼容定义，
  generated widget iframe 也提供 alias fallback，避免影响 app 内部旧主题、历史 CSS 和已生成
  widget 内容。payload 中仍保留更可能被历史生成内容读取的核心 background/bg/text 兼容名、
  `--radius-*` 和 `--spacing-*` 兼容面。
