...
`409` 个 web-ui 唯一颜色是前端生产文件的全域审计数，不是普通 app UI 的色值预算。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `src/web-ui/src` | 627 | 409 | 0 | 0 | 0 | 13 | 主应用完整主题、专用 palette、widget payload 和 editor/terminal/Mermaid 域。 |
| `src/mobile-web/src` | 36 | 31 | 1 | 0 | 0 | 3 | 仅保留图片缩略图黑色 overlay 上的 `#fff` 关闭按钮前景；现有 token 无法同时保证亮暗主题安全。 |
| `BitFun-Installer/src` | 90 | 73 | 0 | 0 | 0 | 2 | 安装器主题数据已压缩为少量 background/accent/status seed 和 tone 派生；runtime 保留实际消费的 accent/purple 色阶和语义状态反馈。 |
...
| generated widget payload key | 98 | widget iframe 对外主题变量 allowlist，作为外部边界单独预算，不计入内部 alias 读取；payload 只保留需要随宿主主题变化的 canonical 颜色、文本、surface、形状、间距、字体、动效和 button component token 子集，host 内部或历史兼容 key 由 iframe fallback/static shell 派生 |
...
| Token contracts | 115 | 101 | `tokens.scss` 等静态契约根；未消费 Sass token、低风险 shadow/overlay/accent alpha 已按 alias 收敛 |
...
| `generatedWidgetPayload.varUnique` | 98 | 98 | 控制 widget 对外主题 payload allowlist 不继续膨胀；该预算只覆盖主题敏感 canonical 子集和 button component 投影，宿主内部和历史兼容 key 不计入 payload API |
...
3. 专用域近似色复核：mobile-web theme preset near pair 已清零；web-ui/installer 已先处理不可感知的跨主题 neutral、tooltip 派生、runtime/static panel 值、极近 overlay fallback，以及 theme preset 的非状态 neutral/弱文本/scene/workbench 值。
...
4. CLI/TUI palette 压缩：CLI/TUI 已纳入 `theme:color-audit:cli`，Rust fallback near pair 归零，preset near pair 已降到 5；后续只处理 preset 内能证明同一终端 surface 语义的近似色，
