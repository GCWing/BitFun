...
`412` 个 web-ui 唯一颜色是前端生产文件的全域审计数，不是普通 app UI 的色值预算。
...
| 专用域需证据复核的 near color pair | 27 |

跨 root 基线必须分别看待；这些数字不是互相累加后的单一上限：

| root | 颜色出现次数 | 唯一颜色 | App UI raw | fallback var | unresolved / non-contract key | dynamic family | 说明 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| `src/web-ui/src` | 630 | 412 | 0 | 0 | 0 | 13 | 主应用完整主题、专用 palette、widget payload 和 editor/terminal/Mermaid 域。 |
...
| 区域 | 当前出现次数 | 当前唯一色数 | 说明 |
| --- | ---: | ---: | --- |
| Theme presets | 171 | 121 | 主题个性与 palette 映射；跨主题深色 neutral、弱文本和非状态浅色背景已收敛；相邻 surface、主题识别主背景、状态色和 editor lineHighlight 继续保留 |
...
| --- | ---: | ---: | --- |
| Theme presets | 0 | 11 | 只剩 light/paper 相邻 surface、Ink Night/Tokyo Night 主题识别 surface、Slate success 状态色和 Monaco lineHighlight 等需视觉证据的 near pair；不按数值继续强合并 |
...
| `colorDomainNearPairs.nearTotal` | 27 | 27 | 控制 theme preset/runtime/token/editor/Mermaid 等专用域 near 队列规模 |
...
| `cli` total unique colors | 153 | 153 | 控制 CLI/TUI preset 和 Rust fallback palette 不继续膨胀 |
| `cli` preset near pairs | 5 | 5 | 记录 CLI preset 内仍需证据复核的近似色，避免无依据回涨 |
