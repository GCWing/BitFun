...
Product Profile 不能替代用户配置；TUI Blueprint 只决定入口投影；用户配置和 Runtime Plugin 都不能启用
...
TUI Blueprint 不携带 renderer、终端句柄、shell helper、GUI key、任意脚本或 Runtime Plugin 状态。Runtime
...
产品内置扩展来自只读产品 source root，随产品升级；用户/工作区插件继续使用独立来源审核、Host trust、
...
- CLI 产物正确消费产品定制 owner 生成的身份、资源、能力、数据和更新投影。
