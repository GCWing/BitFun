...
| CLI-P2 配置与插件管理 | OC-R1 | 直接读取或导入 OpenCode 配置，发现全局/项目插件，展示来源、准备、更新和回退状态。 |
| CLI-P2 可执行服务插件 | OC-R2/OC-R3 | BitFun 管理脚本进程和兼容门面，支持真实工具、依赖和稳定 Hook；用户可按需收紧权限。 |
...
外部资产有“兼容来源”和“显式导入”两种消费方式。兼容来源默认只读已有 OpenCode 文件并生成可解释结果，
无需先迁移；显式导入用于用户希望把资产写入 BitFun 原生配置的场景。CLI-P0 截止到 Dry-run，CLI-P1 才允许
对受支持的非执行型配置执行 apply：
...
来源范围、内容摘要和导入时间。已导入字段以 BitFun 原生配置为准，不再重复应用外部值；外部来源变化时提示
重新导入并展示差异，不做双向写回。
...
- 本地插件管理：安装、启用、停用、更新和卸载状态清楚分离；来源仍启用且旧版本仍合规时，代码或依赖更新失败
  不破坏上一个可用版本；停用、删除、来源撤销、权限收紧或安全策略失效必须撤下旧贡献，来源缺失或损坏时仍
  可清理残留状态。
...
4. 外部配置可以作为只读兼容来源直接生效，也可以显式导入 BitFun 原生配置；凭据和插件执行分别处理。
