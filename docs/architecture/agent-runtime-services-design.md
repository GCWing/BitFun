...
- 接收产品入口选择的唯一 `DeliveryProfile` 和构建期生成的产品组装结果，校验对应 `SurfaceContract`，
  派生静态能力计划、适配器清单和服务提供方；不得读取原始品牌资源、运行构建脚本或再次选择交付形态。
...
// 构建期产物只暴露运行时实际需要的引用；原始产品定义和品牌资源不进入运行时。
pub struct ProductAssemblyResultRef {
...
pub struct CapabilityPlan {
    ...
    pub required_services: Vec<ServiceCapabilityId>,
    pub bundled_extensions: Vec<BundledExtensionRef>,
}

pub struct ProductAssemblyPlan {
    pub product: ProductAssemblyResultRef,
...
`ProductAssemblyPlan` 只保存静态能力、依赖和服务要求，不保存动态健康或权限状态。产品组装把计划交给现有
能力服务，由能力服务结合 provider health 和当前策略产生入口可读的可用状态。TUI、Exec、管理命令和其他入口
消费同一结果，不读取插件主机内部状态。

产品定义不承载用户 Runtime Configuration、凭据或任意构建脚本。`ProductAssemblyResultRef` 的内容摘要
标识已校验的构建结果，不用 schema 版本代替内容版本。产品定义、品牌资源、界面布局和内置扩展以
...
- 适配器 capability/factory 由产品组装选择；`src/crates/assembly/core` 的 `plugin_runtime` 运行时组合点构造具体
  adapter trait object 并注入 Host。Host 不加载具体生态模块，动态来源也不进入产品组装输入。
  OpenCode Hook 可按稳定接口顺序变换输入或输出，最后由对应归属模块校验并提交；ACP 桥接仍按协议适配器进入对应入口。
- unsupported / temporarily-unavailable 通过能力服务的稳定可用状态表达，不让运行时内核读取产品形态。
...
`ProductAssemblyResultRef`、`CapabilityPack` 和 `CapabilityPlan` 在本文件中只作为目标态草图出现，不构成稳定接口定义。稳定边界以
...
- 插件运行时主机 / 兼容适配器能把外部插件接口映射为 BitFun 的工具、受控钩子、工作流贡献和
  能力/副作用声明，并受安全控制面约束。
