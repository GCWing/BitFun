...
- 已建立 `bitfun-services-core`、`bitfun-services-integrations`、`bitfun-agent-tools`、`tool-runtime`、`bitfun-tool-packs`、`bitfun-agent-runtime`、`bitfun-agent-workflows`、`bitfun-runtime-services`、`bitfun-harness`、`bitfun-product-domains`、`bitfun-product-capabilities` 等归属 crate；命名工作流策略由最小的 `bitfun-agent-workflows` 承载，通用 Harness 注入与查询契约继续由 `bitfun-harness` 提供。
