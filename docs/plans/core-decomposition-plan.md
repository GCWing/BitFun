...
17. 已完成：remote-connect runtime 迁移前快照。已基于当前 port baseline 记录 remote session restore、active turn、cancel、image context、tracker event、queue/event fanout 的输入输出和验证命令；后续只有在这些 focused regression 继续通过时才允许移动 runtime owner，不能把 generic attachment guard 当作已接入多模态行为。
