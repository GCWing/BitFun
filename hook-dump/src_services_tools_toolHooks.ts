FILE: E:\Yuanban\cc-haha-src\src\services\tools\toolHooks.ts
LINES: 652
==========
import {
  ...
  try {
    ...
    for await (const result of executePostToolUseFailureHooks(
      ...
      try {
        ...
        // Skip hook_blocking_error in result.message 鈥?blockingError path
 ...
 * deny rules or non-bypass ask rules 鈥?checkRuleBasedPermissions still applies
 * (inc-4788 analog).
...
export async function resolveHookPermissionDecision(
  ...
  if (hookPermissionResult?.behavior === 'allow') {
    ...
    // Hook provided updatedInput for an interactive tool 鈥?the hook IS the
    ...
    // ask rule 鈥?dialog required despite hook approval
    logForDebugging(
  ...
  // No hook decision or 'ask' 鈥?normal permission flow, possibly with
