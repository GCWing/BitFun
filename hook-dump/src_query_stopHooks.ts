FILE: E:\Yuanban\cc-haha-src\src\query\stopHooks.ts
LINES: 591
==========
...
> {
  ...
  try {
    ...
    // generator yields 鈥?preventContinuation comes first, but blockingError.command
    ...
    for await (const result of generator) {
      ...
      if (result.preventContinuation) {
        ...
        if (shouldLetGoalPromptHookContinue(result)) {
          // Goal hook wants to block-and-continue 鈥?don't prevent.
          // The blockingError (in this or a later yield) drives loop continuation.
        } else if (result.blockingError?.command) {
          // Has a blockingError.command but it's NOT a goal hook 鈫?prevent immediately
          ...
          // No blockingError yet 鈥?defer the decision. The goal hook's
    ...
    // the goal hook's signal 鈥?it wants to block-and-continue, not stop.
    ...
    if (pendingPreventContinuation && goalBlockingErrorSeen) {
      // Don't set preventedContinuation 鈥?the blockingErrors will drive
      // loop continuation in query.ts.
      pendingPreventContinuation = null
    } else if (pendingPreventContinuation) {
      // No goal hook blockingError was found 鈫?this preventContinuation
      // is from a regular hook 鈫?actually prevent and stop the session.
      preventedContinuation = true
